...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        true_signature = signature.first
        true_expressions = Link(signature.rest, expressions.rest)
        procedure = do_lambda_form(true_expressions, env)
        env.define(true_signature, procedure)
        return true_signature
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        while curr_expr is not nil:
            result = scheme_eval(curr_expr.first, env)
            if is_scheme_false(result):
                return scheme_eval(curr_expr.first, env)
