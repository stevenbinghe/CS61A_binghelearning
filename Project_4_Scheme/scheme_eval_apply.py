...
from link import *
from scheme_forms import do_mu_form
...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        arg = map_link(lambda exp: scheme_eval(exp, env), rest)
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        while curr_arg is not nil:
            arguments.append(curr_arg.first)
...
def eval_all(expressions, env):
    ...
    if curr_expr is nil:
        return None
    while curr_expr.rest is not nil:
        scheme_eval(curr_expr.first, env)
        curr_expr = curr_expr.rest
    return scheme_eval(curr_expr.first, env) # replace this with lines of your own code
