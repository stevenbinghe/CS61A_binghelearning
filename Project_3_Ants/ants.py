    ...
    def add_to(self, place: Place):
        if place.ant is None:
            ...
            if self.is_container and place.ant.is_container == False:
                ...
                place.ant = self
            elif not self.is_container and place.ant.can_contain(self):
                place.ant.store_ant(self)
            else:
                assert place.ant.can_contain(self) or (not place.ant.is_container and self.is_container), 'Too many ants in {0}'.format(place)
            # END Problem 8b
        ...
        while p.exit is not None:
            ...
            if p.ant is not None:
                if p.ant.is_container:
                    if not p.ant.doubled:
                        p.ant.damage = p.ant.damage * 2
                        p.ant.doubled = True
                    if p.ant.ant_contained is not None:
                        if not p.ant.ant_contained.doubled:
                            p.ant.ant_contained.damage = p.ant.ant_contained.damage * 2
...
D
        ...
        while p is not None and cnt < self.upper_bound + 1:
            if p.is_hive:
                return None
            if p.bees:
    ...
    def action(self, gamestate: GameState):
        if self.cooldown == 0:
            if self.place.bees:
                ...
                self.cooldown = self.chew_cooldown+1
