...
def about(keywords: list[str]):
    ...
    "*** YOUR CODE HERE ***"
    def ab(paragraph):
        after_split = split(lower(remove_punctuation(paragraph)))
        for p in after_split:
            for k in keywords:
                if p==k:
                    return True
        return False
...
def autocorrect(entered_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    for word in word_list:
        if entered_word == word:
            return word
        if diff > diff_function(entered_word, word, limit):
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    """
    # BEGIN PROBLEM 6
    if len(entered)>len(source):
        return len(entered)-len(source)+furry_fixes(entered[:-(len(entered)-len(source))], source, limit)
    if len(entered)<len(source):
        return len(source)-len(entered)+furry_fixes(entered, source[:-(len(source)-len(entered))], limit)
...
def report_progress(entered: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    "*** YOUR CODE HERE ***"
    success = 0
    while success < len(source):
        if success == len(entered):
            a = {'id':user_id, 'progress':success/len(source)}
            upload(a)
            return success/len(source)
        if entered[success] == source[success]:
            success += 1
        else:
            a = {'id': user_id, 'progress': success / len(source)}
            upload(a)
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    for t in ts_by_player:
        per_time = []
        for i in range(len(words)):
            per_time = per_time + [t[i+1] - t[i]]
        times = times + [per_time]
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    "*** YOUR CODE HERE ***"
    results = []
    for player in pl_idxs:
        results = results + [[]]
    for word in w_idxs:
        winner = 0
        winner_time = get_time(times, pl_idxs[0], word)
        for player in pl_idxs:
            if winner_time > get_time(times, player, word):
                winner_time = get_time(times, player, word)
                winner = player
        results[winner].append(words[word])
    return results
