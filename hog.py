...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    total=3*abs(player_score%10-((opponent_score-(opponent_score%10))%100)/10)
    if total == 0:
        return 1
    else:
        return int(total)
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score[who]<goal and score[1-who]<goal:
        num_rolls=strategy[who](score[who],score[1-who])
        score[who]=update(num_rolls,score[who],score[1-who],dice)
        who = 1-who
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while a < 11:
        ...
        # if averaged_dice(a,dice) > max:
        #     max = averaged_dice(a,dice)调用了两次，现改为只调用一次
        score = averaged_dice(a, dice)
        if score > max:
            max = score
            num = a
        a += 1
...
def self_max_scoring_num_rolls(dice=six_sided, times_called=1000):
    a=1
    max=0
    num = 0
    averaged_dice = make_averaged(roll_dice, times_called)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    if  a < threshold:
        if sus_points(score+a)-score < threshold:
            return num_rolls
    return 0
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    num_rolls , threshold = self_max_scoring_num_rolls(dice=six_sided, times_called=1000)
    if opponent_score < GOAL - threshold:
